      ...
      //初始化
		init: function () {
			...
			silderPage_li.on('click',function(){
				...
				//自行代码
				$("#demo3 #silder_page li").removeClass("a_backImg");
				$(this).addClass("a_backImg");
			});
			...
			silderList.hover(function(){window.clearInterval(t); return;},function(){ t = window.setInterval(function(){
				if(pagesize < img_size && pagesize >= 0)
				{
					...
					//自写代码
					$("#demo3 #silder_page li").removeClass("a_backImg");
					//原始代码 silderPage_li.eq(pagesize).addClass('current').siblings().removeClass('current');
					silderPage_li.eq(pagesize).addClass('current a_backImg').siblings().removeClass('current');
					if(opt.is_showTit){
						$('#silder_desc').text(silderList_li.eq(pagesize).find('img').attr('alt'));
					}
					pagesize++;
			...
			},opt.s_times);}).trigger("mouseout"); //悬浮时 停止自动动画,trigger 起默认触发作用 自动轮播: mouseout
